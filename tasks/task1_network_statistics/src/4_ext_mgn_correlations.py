...
B2B_df_full['ln_turnover'] = np.log(B2B_df_full['turnover'])

# Estimate the FE model
B2B_df_full.set_index(['nace','year'],inplace=True)
...
for year in pd.unique(B2B_df['year']):
    ...
    df_year['ln_turnover'] = np.log(df_year['turnover'])
    
    # Estimate the FE model
    df_year.set_index(['nace','year'],inplace=True)
