...
# modify the firms' data to include broad definition of industries
# Preprocess the firms_df to extract the first two characters of NACE codes and convert to integers
firms_df['nace_2digit'] = firms_df['nace'].astype(str).replace(
...
firms_df['industry'] = firms_df['nace_2digit'].apply(recategorize_industry)

# merge network and firm data
B2B_df = B2B_df.merge( firms_df[['year','vat','nace','industry']].rename(
    columns={'vat': 'vat_i', 'industry': 'industry_i', 'nace':'nace_i'}), on=['vat_i','year'], how='inner' )
B2B_df = B2B_df.merge( firms_df[['year','vat','nace']].rename(
    columns={'vat': 'vat_j', 'nace': 'nace_j'}), on=['vat_j','year'], how='inner' )
...
for year in pd.unique(B2B_df['year']):
    ...
    for industry in industries:
        industry_nodes = df_year[df_year['industry_i'] == industry]['vat_i'].unique()
        out_degree_sec = dict( G_year.out_degree(industry_nodes) )
    ...
    for industry in industries:
        industry_nodes = df_year[df_year['industry_i'] == industry]['vat_i'].unique()
        in_degree_sec = dict( G_year.in_degree(industry_nodes) )
